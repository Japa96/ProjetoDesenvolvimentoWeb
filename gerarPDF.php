...
$html = "";
        ...
        if ($rs->execute($filtro)) {
          if ($rs->rowCount() > 0) {
            ...
            $html .= "Descrição : ".$row->observacao. "<hr>";
            $html .= '<h3>Pedido</h3>';

            $html .= "Produto : ".$row->descricao. "<br>";
            $html .= "Valor : ".$row->valorUnitario. "<br>";
            $html .= "Quantidade : ".$row->quantidade. "<br>";
            $html .= "Descrição : ".$row->obsPedido. "<br>";
        ...
        $dompdf = new DOMPDF();
        $dompdf->load_html('
            <h2 style="text-align: center;">Compra</h2>
            <h3>Informações</h3>
            '.$html.'
        ');
        $dompdf->render();
